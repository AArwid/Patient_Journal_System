...
beforeAll(() => {
  patient = patientsRepository.create({ fullName: 'Erik E', personalNumber: 'p-4' });
  usersRepository.create({ name: 'Doc', email: 'doc3@test.com', passwordHash: hashPassword('pw'), role: 'doctor' });
...
describe('audit log', () => {
  it('records a granted view_journal event, visible via the access-logs endpoint', async () => {
    ...
    const granted = res.body.logs.find(
      (block) => block.event.type === 'view_journal' && block.event.outcome === 'granted'
    );
  ...
  it('records a denied attempt from the unauthorized role', async () => {
    ...
    const denied = res.body.logs.find(
      (block) => block.event.actorRole === 'unauthorized' && block.event.outcome === 'denied'
    );
